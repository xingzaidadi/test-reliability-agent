# 从一个 AI 到一支 AI 测试团队:一个靠不住,那就让两个互相盯着
...
AI 也一样。**解法不是"找一个更聪明的 AI",而是"组织多个 AI 分工制衡"。** 这正是某开源 agent 编排平台 这类平台的思路——把 coding agent 当团队成员,分配任务、追踪、协作。该控制框架 也有一条原则:**控制面必须随 agent 能力演进**——当 agent 能协作、能反向调用,你的编排系统就得管得住这种协作。

---

## 二、约束下的选择:为什么自研

我本想直接用 某开源编排平台。但撞上两个硬约束:

- **公司不让装 Docker**(某开源编排平台 自托管要 Docker)
- **数据不能出网**(某开源编排平台 连云要把任务发到外部)

这两条都是红线。于是我做了个决定:**借鉴 某开源编排平台 的理念,自研一个纯本地的轻量编排器。**
...
编排器的核心,是把"AI 协作"抽象成几个概念(借鉴 某开源编排平台):
...
**关键设计:能力做成可插拔的 Skill。** 加一个新测试能力,就是注册一个 Skill,不用改核心代码——同事也能维护。这正是 某开源编排平台 "compound skills"(技能复用)的思路。
...
多 agent 协作带来一个新风险:**agent 之间可能形成死循环**(A 触发 B,B 又触发 A)。该控制框架 专门做了循环检测(REPEAT/PINGPONG/BURST 三种模式熔断)。
...
**硬约束优于软约束**(工程化原则:硬约束优于软约束):不是提示 AI "别循环",而是机制上让它循环不起来。
...
- 我实现了任务级的 DAG 成环检测(硬约束);该控制框架 的 MCP 循环检测(REPEAT/PINGPONG/BURST 语义级)更进一步,是我的下一步。
