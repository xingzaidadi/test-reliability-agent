...
我本想直接用 现成编排平台。但撞上两个硬约束:

- **公司不让装 Docker**(现成编排平台 自托管要 Docker)
- **数据不能出网**(现成编排平台 连云要把任务发到外部)

这两条都是红线。于是我做了个决定:**借鉴 现成编排平台 的理念,自研一个纯本地的轻量编排器。**
...
编排器的核心,是把"AI 协作"抽象成几个概念(借鉴 现成编排平台):
...
**关键设计:能力做成可插拔的 Skill。** 加一个新测试能力,就是注册一个 Skill,不用改核心代码——同事也能维护。这正是 现成编排平台 "compound skills"(技能复用)的思路。
