...
> **上游前置:测试数据构造。** 没数据测不了(接口只会返回"未找到")。测试数据的自动构造由另一个独立项目 **[DataForge Loop](#)**(AI 数据智造闭环引擎)承担——从需求/SOP 截图自动造跨系统供应链测试数据。两个项目接力:DataForge 造数据,本平台用数据把测试跑严跑全。

| 能力 | 说明 |
|---|---|
| **API 功能测试** | 读 YAML 用例 → 发真实 HTTP → 求值断言。支持网关签名协议(签名 + 信封 + 编码封装)。 |
